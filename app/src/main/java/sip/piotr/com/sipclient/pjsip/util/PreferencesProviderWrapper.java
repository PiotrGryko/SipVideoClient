...
package sip.piotr.com.sipclient.pjsip.util;
...
public class PreferencesProviderWrapper {
    ...
    private void init()
    {
        HashMap<String,String> stringPrefs = PreferencesWrapper.STRING_PREFS;
        Iterator<String> iterator = stringPrefs.keySet().iterator();
        while (iterator.hasNext())
        {
            String key = iterator.next();
            setPreferenceStringValue(key, stringPrefs.get(key));
        }

        HashMap<String,Boolean> booleanPrefs = PreferencesWrapper.BOOLEAN_PREFS;
        iterator = booleanPrefs.keySet().iterator();
        while (iterator.hasNext())
        {
            String key = iterator.next();
            setPreferenceBooleanValue(key, booleanPrefs.get(key));
        }
        HashMap<String,Float> floatPrefs = PreferencesWrapper.FLOAT_PREFS;
        iterator = floatPrefs.keySet().iterator();
        while (iterator.hasNext())
        {
            String key = iterator.next();
            setPreferenceFloatValue(key, floatPrefs.get(key));
        }
    }
...
//        resolver.update(uri, new ContentValues(), null, null);
    ...
    public boolean getPreferenceBooleanValue(String string, boolean b) {
        b= SipConfigManager.getPreferenceBooleanValue(context, string, b);
        Log.d(TAG,"get value key= "+string +", value= "+b);
        return b;
    }

    public Boolean getPreferenceBooleanValue(String string) {

        boolean b =  SipConfigManager.getPreferenceBooleanValue(context, string);
        Log.d(TAG,"get value key= "+string +", value= "+b);
        return b;
    }

    public String getPreferenceStringValue(String key) {
        String b =  SipConfigManager.getPreferenceStringValue(context, key);
        Log.d(TAG,"get value key= "+key +", value= "+b);
        return b;
    }

    public String getPreferenceStringValue(String key, String defaultVal) {
        String b =SipConfigManager.getPreferenceStringValue(context, key, defaultVal);
        Log.d(TAG,"get value key= "+key +", value= "+b);
        return b;
    }

    public int getPreferenceIntegerValue(String key) {
        int b = SipConfigManager.getPreferenceIntegerValue(context, key);
        Log.d(TAG,"get value key= "+key +", value= "+b);
        return b;
    }

    public float getPreferenceFloatValue(String key) {
        float b =  SipConfigManager.getPreferenceFloatValue(context, key);
        Log.d(TAG,"get value key= "+key +", value= "+b);
        return b;
    }

    public float getPreferenceFloatValue(String key, float f) {
        float b= SipConfigManager.getPreferenceFloatValue(context, key, f);
        Log.d(TAG,"get value key= "+key +", value= "+b);
        return b;
    }

    public void setPreferenceStringValue(String key, String newValue) {
        Log.d(TAG,"set value key= "+key +", value= "+newValue);
    ...
    public long getClockRate(MediaManager mediaManager) {
        ...
        try {
            ...
            rate = defaultRate;
            Log.d("XXX","get reate "+rate);
            return rate;
        } catch (NumberFormatException e) {
            Log.e(THIS_FILE, "Clock rate " + clockRate + " not well formated");
        }
        Log.d("XXX","get default rate" +defaultRate);
        return defaultRate;
    }
