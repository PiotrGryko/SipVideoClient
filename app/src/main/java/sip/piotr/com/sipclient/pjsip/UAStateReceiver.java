...
import sip.piotr.com.sipclient.api.SipMessage;
...
import sip.piotr.com.sipclient.pjsip.service.impl.SipCallSessionImpl;
import sip.piotr.com.sipclient.pjsip.util.CallLogHelper;
import sip.piotr.com.sipclient.pjsip.util.Compatibility;
...
public class UAStateReceiver extends Callback {
    ...
    public void on_incoming_call(final int accId, final int callId, SWIGTYPE_p_pjsip_rx_data rdata) {
        Log.d(THIS_FILE,"on incoming call ");
    ...
    public void on_call_state(final int callId, pjsip_event e) {
        ...
        Log.d(THIS_FILE, "!!!!!!!!!!!!!!!!!!!Call state change  id is "+callId);
        try {
            ...
            int cachedInvState = callInfo.getCallState();
            int cachedMediaState = callInfo.getMediaStatus();
            boolean cachedCanRecord = callInfo.canRecord();
            boolean cachedIsRecording = callInfo.isRecording();
            boolean cachedIsHold = callInfo.isLocalHeld();
            boolean cachedVideo = callInfo.mediaHasVideo();
            boolean cachedZrtpActive = callInfo.getHasZrtp();
            boolean cachedZrtpVerified = callInfo.isZrtpSASVerified();

            Log.d(THIS_FILE, "Call state  = "+callState);
...
//                binfo.getMonitor_pres(), presStatus, presStatusTxt);
    ...
    public void on_call_media_state(final int callId) {
        ...
        Log.d(THIS_FILE,"on call media state "+callId);
        ...
        try {
            ...
            Log.d(THIS_FILE,"call data updated  "+callId +" "+callInfo.getCallState());
     ...
     * @throws SipService.SameThreadException if we are calling that from outside the pjsip
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case ON_CALL_STATE: {
                    ...
                    Log.d(THIS_FILE,"handler on call sate "+callInfo.getCallState());
                    ...
                    switch (callState) {
                        ...
                        case SipCallSession.InvState.CONFIRMED:
                            ...
                            if (callState == SipCallSession.InvState.CONFIRMED) {
                                Log.d(THIS_FILE,"auto pending dtmf");
    ...
    public void initService(PjSipService srv) {
        ...
        Log.d(THIS_FILE,"service initialised");
        pjService = srv;
    ...
    private void broadCastAndroidCallState(String state, String number) {
        ...
        Log.d("TTT","Broadcasting state "+state +" "+number);
    ...
    public void updateCallMediaState(int callId) throws SipService.SameThreadException {
        SipCallSession callInfo = updateCallInfoFromStack(callId, null);
        Log.d(THIS_FILE,"update call media state "+callId +" "+callInfo.getCallState());
        msgHandler.sendMessage(msgHandler.obtainMessage(ON_MEDIA_STATE, callInfo));
    }
}
